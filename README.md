...
## Еще один возможный сценарий (хотя очень маловероятный если честно).

1. Если пользователь авторизовался (login), но refresh token еще не использовался и он не вышел из аккаунта (не сделал logout), то есть токен не был удален из массива refreshToken в БД;
2. При этом refresh token был скомпроментирован (украден);
3. Если случилось 1 и 2, то это как раз слаучай reuse detection и можно удалить все токены из массива refreshToken в БД, когда пользователь авторизуется заново.

Если пользователь заново авторизуется на сайте, но его предыдущего refresh токена из cookies уже нет в массиве в БД (а в штатной ситуации он должен там быть), значит его уже пытались переиспользовать (сработал Reuse Detection в `/refresh`). В этом случае можно удалить все refresh token'ы в БД. Реализация в `server/controllers/authController.js` 42 строчка.

[Youtube: Refresh Token Rotation and Reuse Detection in Node.js JWT Authentication](https://www.youtube.com/watch?v=s-4k5TcGKHg)
